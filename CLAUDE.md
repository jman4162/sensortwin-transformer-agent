...
- **Results + viz** — the 3-seed `colab_standard` comparison (transformer 0.900 ± 0.003 beats the best
  baseline by +0.056, p=0.006) is the README headline (quick-demo demoted to a small-scale contrast).
  Reproduce via `notebooks/04_colab_standard_comparison.ipynb`; follow-ups in
  `notebooks/{05_calibration,06_label_efficiency,07_robustness}_colab.ipynb`. Committed portfolio
  figures live in `docs/figures/` (not gitignored), regenerated by `scripts/make_figures.py` and
  rendered by new `evaluation/plots.py` helpers (`plot_signal_gallery`/`plot_scale_comparison`/
